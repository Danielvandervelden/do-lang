...
- **Self-review:** CHANGES_REQUESTED — stale prose at `stage-project-intake.md:290` still listed `slug-as-title` as a PI-6b failure category after iter-15 removed the check.
- **Council (codex):** CHANGES_REQUESTED — architectural finding: parent-index (`project.md.phases[]` / `phase.md.waves[]`) is seeded once by `project-scaffold.cjs` at node creation with `{slug, status:'planning'}` but never re-synced by `project-state.cjs` on subsequent transitions. β's control-flow reads at five sites (`skills/do/project.md` phase-complete step 1, phase-complete step 5, wave-next step 2; `stage-project-complete.md` PC-1 and PC-2) treat the parent index as authoritative, so they see stale-or-zero data after any leaf mutation. Next-phase selection, project-complete precondition, and γ-gate would all operate on snapshot-at-creation state.
- **User directive context:** This is a real β-contract bug — β's markdown prescribes reads that don't reflect current state. Not implementation-phase deferrable. Two fix options: (A) extend α's `project-state.cjs` to propagate status writes to parent indexes (α-scope), or (B) switch β's reads to leaf files and make parent-index advisory-only (β-scope). Chose (B) to keep β inside its charter and avoid α code changes.
- **Action:**
  1. Added a new `## Authoritative state reads` section near the top of `skills/do/project.md` explicitly declaring the parent-index advisory. Reads for control flow must walk the filesystem and read leaf files (`.../phases/<slug>/phase.md`, `.../waves/<slug>/wave.md`), sort lexically by NN-prefixed slug. Documents the future-work path: α's `project-state.cjs` could be extended to sync, but β doesn't depend on it.
  2. Rewrote five parent-index reads to walk `fs.readdirSync(phasesDir)` / `fs.readdirSync(wavesDir)` and read leaf frontmatter directly:
     - `skills/do/project.md` phase-complete step 1 (completed-wave precondition check)
     - `skills/do/project.md` phase-complete step 5 (next-phase identification)
     - `skills/do/project.md` wave-next step 2 (first-planning-wave identification)
     - `stage-project-complete.md` PC-1 (all-phases-completed precondition)
     - `stage-project-complete.md` PC-2 (γ-gate handoff-existence check)
     All five now skip the stale parent index and each carries a short inline note + pointer to §Authoritative state reads.
  3. Removed dual-write burden from out-of-scope option in `stage-wave-verify.md` Option 4 (dropped step 3: "Update parent phase.md waves[] index") and `stage-wave-exec.md` BLOCKED path option 3c (same). These writes were cosmetic — now documented as unnecessary because reads go to leaves. Renumbered the post-removal steps. `project-state.cjs` leaf writes remain as-is (authoritative).
  4. Downgraded `phaseStatusDrift` and `waveStatusDrift` severity from `warning` to `info` in `init-health-check.md` with a note that drift is expected under advisory-only semantics (parent-index is a snapshot for display; reconciliation is optional).
  5. Fixed self-reviewer's `slug-as-title` stale-prose nitpick in `stage-project-intake.md:290`.
