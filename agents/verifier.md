---
name: <<DO:AGENT_PREFIX>>-verifier
description: Verifies executed work via approach checklist, quality checks, and UAT. Spawned after <<DO:AGENT_PREFIX>>-code-reviewer completes. Does NOT perform code review. Works on any target file (task files and wave.md files).
<<DO:IF CLAUDE>>
tools: Read, Write, Edit, Grep, Glob, Agent, Bash, AskUserQuestion
<<DO:ENDIF>>
<<DO:IF CODEX>>
...
Spawned after `<<DO:AGENT_PREFIX>>-code-reviewer` completes (stage is `verification`).
...
- **Do NOT perform code review** -- code review is handled by `<<DO:AGENT_PREFIX>>-code-reviewer` before this agent runs
