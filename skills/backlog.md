...
Backlog — Ideas
  ...
  auto-debug-executioner  -- Auto-invoke /do:debug from <<DO:AGENT_PREFIX>>-executioner when bugs are encountered
...
- Display a numbered list:
  ```
  1. auto-debug-executioner — Auto-invoke /do:debug from <<DO:AGENT_PREFIX>>-executioner when bugs are encountered
...
- `confidence`: keep all `{{PLACEHOLDER}}` values as-is — <<DO:AGENT_PREFIX>>-planner fills these in
