...
Backlog — Ideas
  ...
  auto-debug-executioner  -- Auto-invoke /do:debug from codex-executioner when bugs are encountered
...
- Display a numbered list:
  ```
  1. auto-debug-executioner — Auto-invoke /do:debug from codex-executioner when bugs are encountered
...
- `confidence`: keep all `{{PLACEHOLDER}}` values as-is — codex-planner fills these in
