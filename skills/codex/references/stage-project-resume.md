...
This reference file is loaded by `skills/codex/project.md` `resume`. It implements the cold-start resume flow per orchestrator §7 steps 1–8.
...
```bash
node ~/.codex/skills/do/scripts/project-resume.cjs
```
...
PROJ_DATA=$(node ~/.codex/skills/do/scripts/update-task-frontmatter.cjs read '.do/projects/<active_project>/project.md' active_phase)
...
if [ -n "$ACTIVE_PHASE" ] && [ -f ".do/projects/<active_project>/phases/$ACTIVE_PHASE/phase.md" ]; then
  PHASE_DATA=$(node ~/.codex/skills/do/scripts/update-task-frontmatter.cjs read ".do/projects/<active_project>/phases/$ACTIVE_PHASE/phase.md" active_wave)
