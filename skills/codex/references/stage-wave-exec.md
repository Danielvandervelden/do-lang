...
```bash
node ~/.codex/skills/do/scripts/update-task-frontmatter.cjs check '<wave_path>' stages.execution==complete
```
...
## WE-2: Spawn codex-executioner

Spawn the codex-executioner subagent with model `<models.overrides.executioner || models.default>` and the description "Execute wave plan". Pass the following prompt:
...
- **BLOCKED**: Surface blocker to user:
  ...
  3. Mark out of scope. Run the same state-machine-legal two-step used by `stage-wave-verify.md` Option 4 (leaf-only writes — the phase-completion check reads `wave.md` directly per `skills/codex/project.md` §Authoritative state reads):
...
```bash
node ~/.codex/skills/do/scripts/update-task-frontmatter.cjs read '<wave_path>' stages.execution
```

If `stages.execution` is `complete`, proceed to WE-4. If not, the executioner may have stopped mid-run.

If codex-executioner set `stages.execution: complete`, proceed to WE-4.
