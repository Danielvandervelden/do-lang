...
description: "Wave plan review block for /do:project. Council gate check, parallel reviewer spawning (when enabled), verdict combination, iteration loop via codex-planner, and escalation rules. Target file: wave.md."
---

# Wave Plan Review Stage

This reference file is loaded by `skills/codex/project.md` `wave next` after the per-wave confidence rescue check. It encodes the full plan review logic for `wave.md`.
...
```bash
node ~/.codex/skills/do/scripts/update-task-frontmatter.cjs check '<wave_path>' council_review_ran.plan
```
...
```bash
node ~/.codex/skills/do/scripts/council-gate.cjs project.wave_plan planning
```
...
```bash
node ~/.codex/skills/do/scripts/update-task-frontmatter.cjs read-body '<wave_path>' | grep -q '{{[A-Z_]*}}'
...
**If placeholders remain (exit 0):** Spawn codex-planner to curate the wave body from phase and project context before reviewers see it:

Spawn the codex-planner subagent with model `<models.overrides.planner || models.default>` and the description "Curate wave plan from phase context". Pass the following prompt:
...
**Wait for codex-planner to complete before proceeding to PR-3.** Reviewers must see curated content, not scaffold placeholders — sending reviewers against a template with `{{PROBLEM_STATEMENT}}` / `{{APPROACH}}` markers would cause an automatic RETHINK verdict on every first pass.
...
Spawn the codex-plan-reviewer subagent with model `<models.overrides.plan_reviewer || models.default>` and the description "Self-review wave plan". Pass the following prompt:
...
Spawn the codex-council-reviewer subagent with model `<models.overrides.plan_reviewer || models.default>` and the description "Council review wave plan". Pass the following prompt:
...
Spawn the codex-plan-reviewer subagent with model `<models.overrides.plan_reviewer || models.default>` and the description "Self-review wave plan (council disabled)". Pass the following prompt:
...
4. Spawn codex-planner with reviewer feedback:
   ...
   Spawn the codex-planner subagent with model `<models.overrides.planner || models.default>` and the description "Revise wave plan based on review feedback". Pass the following prompt:
