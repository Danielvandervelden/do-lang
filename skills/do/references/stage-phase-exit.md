...
This reference file is invoked by `skills/do/project.md` `phase complete` **after** the state transition has completed (i.e., after β has set the completed phase to `status: completed`, cleared `active_wave` and `active_phase`, and appended to `changelog.md`).
