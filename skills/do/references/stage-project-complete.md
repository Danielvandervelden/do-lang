...
These artefacts are generated by `/do:project phase complete` once Task γ
(project-gamma-resume-handoff) is shipped. Without γ, the hot-session flow
works end-to-end but project archival is unavailable.

To proceed: wait for Task γ to be deployed, run `/do:project phase complete`
for each phase to generate handoff.md files, then re-run `/do:project complete`.
```
