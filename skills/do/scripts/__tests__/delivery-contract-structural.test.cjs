 ...
 *
 * Run: node --test skills/do/scripts/__tests__/delivery-contract-structural.test.cjs
 */
...
const SCRIPTS_DIR = path.join(ROOT, 'skills', 'do', 'scripts');
...
describe('delivery-contract.md: exists and contains schema field definitions', () => {
  ...
  before(() => {
    assert.ok(fs.existsSync(filePath), 'delivery-contract.md must exist in skills/do/references/');
...
describe('validate-delivery-contract.cjs: exists and exports required functions', () => {
  ...
  before(() => {
    assert.ok(fs.existsSync(filePath), 'validate-delivery-contract.cjs must exist in skills/do/scripts/');
...
// ============================================================================
// do-executioner.md — Delivery Contract awareness
// ============================================================================

describe('do-executioner.md: references Delivery Contract in execution context', () => {
  const filePath = path.join(AGENTS_DIR, 'do-executioner.md');
  let content;

  before(() => {
    assert.ok(fs.existsSync(filePath), 'do-executioner.md must exist');
    content = read(filePath);
  });
  ...
  it('Step 1 Load Execution Context mentions Delivery Contract', () => {
    assert.ok(
      content.includes('Delivery Contract'),
      'do-executioner.md must reference Delivery Contract in Step 1'
    );
  });
  ...
  it('contains branch mismatch as a blocking deviation', () => {
    assert.ok(
      ...
      'do-executioner.md must document branch mismatch as a blocking deviation'
    );
  });
  ...
  it('contains exclude_paths enforcement', () => {
    assert.ok(
      ...
      'do-executioner.md must reference exclude_paths enforcement'
    );
  });
});
...
describe('delivery-onboarding.md: exists and contains the three onboarding options', () => {
  ...
  before(() => {
    assert.ok(fs.existsSync(filePath), 'delivery-onboarding.md must exist in skills/do/references/');
    content = read(filePath);
  });
