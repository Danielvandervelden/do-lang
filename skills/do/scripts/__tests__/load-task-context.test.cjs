 ...
 * Run: node --test skills/do/scripts/__tests__/load-task-context.test.cjs
 */
...
try {
  ...
  extractKeywords = () => { throw new Error('Module not implemented'); };
  findMatchingDocs = () => { throw new Error('Module not implemented'); };
  loadTaskContext = () => { throw new Error('Module not implemented'); };
}
...
describe('extractKeywords', () => {
  ...
  test('returns lowercase deduplicated array', () => {
    ...
    assert.strictEqual(result.length, unique.length, 'should have no duplicates');
    result.forEach((kw) => {
      assert.strictEqual(kw, kw.toLowerCase(), `keyword "${kw}" should be lowercase`);
    });
  });

  test('includes words longer than 5 characters', () => {
    const result = extractKeywords('Implement authentication feature');
    assert.ok(result.includes('implement'), 'should include "implement" (>5 chars)');
    assert.ok(result.includes('authentication'), 'should include "authentication"');
    assert.ok(result.includes('feature'), 'should include "feature" (>5 chars)');
  });
  ...
  test('excludes short generic words', () => {
    ...
    assert.ok(!result.includes('fix'), 'should not include "fix" (only 3 chars, not a tech term)');
  });
});

describe('findMatchingDocs', () => {
  test('returns empty array when database path does not exist', () => {
    const result = findMatchingDocs('/nonexistent/path', ['form', 'validation']);
...
describe('loadTaskContext', () => {
  test('returns error when workspace not initialized', () => {
    ...
    assert.ok(Array.isArray(result.keywords), 'should always have keywords array');
  });
