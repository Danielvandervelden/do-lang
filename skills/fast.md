...
The full `/do:task` workflow (5-7 <<DO:CLAUDE:agent spawns>><<DO:CODEX:subagent spawns>> + review rounds) is disproportionate for trivial 1-3 file changes. `/do:fast` removes ceremony while keeping context scan, execution, validation, and single code review.
