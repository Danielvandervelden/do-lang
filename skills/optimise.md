...
node <<DO:SCRIPTS_PATH>>/optimise-target.cjs "<target_path>" --effort <effort>

# Target only (medium effort):
node <<DO:SCRIPTS_PATH>>/optimise-target.cjs "<target_path>"

# Effort only (project mode):
node <<DO:SCRIPTS_PATH>>/optimise-target.cjs --effort <effort>

# No args (project mode, medium effort):
node <<DO:SCRIPTS_PATH>>/optimise-target.cjs
```
