...
1. Resolve the `council-invoke.cjs` path (installed or dev):
   - Installed: `<<DO:SCRIPTS_PATH>>/council-invoke.cjs`
...
4. Run `parseCouncilRunnerOutput()` on the council agent's output text (the flattened `VERDICT: ...\nAdvisor: ...\nFindings:\n- ...\nRecommendations:\n- ...` block from <<DO:AGENT_PREFIX>>-council-reviewer). Do NOT use `parseFindings()` — that function parses raw advisor markdown with `### Key Findings` headers, not council runner output.
