...
This reference file is invoked by `skills/project.md` `phase complete` **after** the state transition has completed (i.e., after β has set the completed phase to `status: completed`, cleared `active_wave` and `active_phase`, and appended to `changelog.md`).
...
3. **All `wave.md` files** within the completed phase — walk `.do/projects/<active_project>/phases/<completed_phase_slug>/waves/` and read each leaf `wave.md` directly (NOT from `phase.md.waves[]`, which is scaffold-seeded and not synced):
   ...
   const fmRead = (f) => JSON.parse(execSync('node <<DO:SCRIPTS_PATH>>/update-task-frontmatter.cjs read \"' + f + '\"', { encoding: 'utf8' }));
...
- One line from `wave_summary` frontmatter field (written by <<DO:AGENT_PREFIX>>-verifier at wave completion).
...
```bash
cat skills/references/handoff-template.md
# or installed path:
<<DO:IF CLAUDE>>
# cat ~/.claude/commands/do/references/handoff-template.md
<<DO:ENDIF>>
<<DO:IF CODEX>>
