<<DO:IF CODEX>>
**Agent authorization:** The caller workflow has authorized spawning <<DO:AGENT_PREFIX>>-griller
...
```bash
node <<DO:SCRIPTS_PATH>>/project-state.cjs check next-planning-phase --project <active_project>
```
...
Read the next phase's `phase.md` confidence score. If below `project_intake_threshold`, spawn `<<DO:AGENT_PREFIX>>-griller` (pass the project threshold explicitly — `<<DO:AGENT_PREFIX>>-griller`'s default is task-safe):
...
<<DO:ENDIF>>
<<DO:IF CODEX>>
Spawn the <<DO:AGENT_PREFIX>>-griller subagent with model `<models.overrides.griller || models.default>` and the description "Per-phase re-grill (Pass 3)". Pass the following prompt:
