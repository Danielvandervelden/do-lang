...
description: Orchestrator plan review block. Council gate check, parallel reviewer spawning (when enabled), verdict combination, iteration loop via <<DO:AGENT_PREFIX>>-planner, and escalation rules.
---
...
```bash
node <<DO:SCRIPTS_PATH>>/council-gate.cjs planning
```
...
<<DO:ENDIF>>
<<DO:IF CODEX>>
Spawn the <<DO:AGENT_PREFIX>>-plan-reviewer subagent with model `<models.overrides.plan_reviewer || models.default>` and the description "Self-review plan". Pass the following prompt:
...
Spawn the <<DO:AGENT_PREFIX>>-council-reviewer subagent with model `<models.overrides.plan_reviewer || models.default>` and the description "Council review plan". Pass the following prompt:
<<DO:ENDIF>>
...
<<DO:ENDIF>>
<<DO:IF CODEX>>
Spawn the <<DO:AGENT_PREFIX>>-plan-reviewer subagent with model `<models.overrides.plan_reviewer || models.default>` and the description "Self-review plan (council disabled)". Pass the following prompt:
...
Load and follow `@references/classify-findings.md`. The self-review output comes from <<DO:AGENT_PREFIX>>-plan-reviewer (CONCERNS or RETHINK response).
...
3. Resolve the `stage-decision.cjs` path (installed or dev):
   - Installed: `<<DO:SCRIPTS_PATH>>/stage-decision.cjs`
...
<<DO:IF CODEX>>
   ...
   Spawn the <<DO:AGENT_PREFIX>>-planner subagent with model `<models.overrides.planner || models.default>` and the description "Revise plan based on review feedback". Pass the following prompt:
