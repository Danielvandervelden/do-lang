...
<<DO:IF CLAUDE>>
Spawn `<<DO:AGENT_PREFIX>>-griller` with the Pass 1 question bank:
...
<<DO:ENDIF>>
<<DO:IF CODEX>>
Spawn the <<DO:AGENT_PREFIX>>-griller subagent with model `<models.overrides.griller || models.default>` and the description "Project intake: Pass 1 grilling (vision + scope)". Pass the following prompt:
...
node <<DO:SCRIPTS_PATH>>/update-task-frontmatter.cjs read '<project_path>' confidence
...
<<DO:IF CLAUDE>>
Spawn `<<DO:AGENT_PREFIX>>-griller` with the Pass 2 question bank:
...
<<DO:ENDIF>>
<<DO:IF CODEX>>
Spawn the <<DO:AGENT_PREFIX>>-griller subagent with model `<models.overrides.griller || models.default>` and the description "Project intake: Pass 2 grilling (phase seed)". Pass the following prompt:
...
CONFIDENCE=$(node <<DO:SCRIPTS_PATH>>/update-task-frontmatter.cjs read '<project_path>' confidence)
...
## PI-6: Curate project.md — Spawn <<DO:AGENT_PREFIX>>-planner

<<DO:IF CLAUDE>>
Spawn `<<DO:AGENT_PREFIX>>-planner` to curate `project.md`'s body sections from the intake transcript:
<<DO:ENDIF>>
<<DO:IF CODEX>>
Spawn the <<DO:AGENT_PREFIX>>-planner subagent with model `<models.overrides.planner || models.default>` and the description "Curate project.md body from intake responses". Pass the following prompt:
...
TITLE=$(node <<DO:SCRIPTS_PATH>>/update-task-frontmatter.cjs read '<project_path>' title)
BODY=$(node <<DO:SCRIPTS_PATH>>/update-task-frontmatter.cjs read-body '<project_path>')
...
```bash
node <<DO:SCRIPTS_PATH>>/project-state.cjs set project <active_project> status=planning
```
