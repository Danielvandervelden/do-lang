...
```bash
node <<DO:SCRIPTS_PATH>>/project-resume.cjs
```
...
PROJ_DATA=$(node <<DO:SCRIPTS_PATH>>/update-task-frontmatter.cjs read '.do/projects/<active_project>/project.md' active_phase)
...
if [ -n "$ACTIVE_PHASE" ] && [ -f ".do/projects/<active_project>/phases/$ACTIVE_PHASE/phase.md" ]; then
  PHASE_DATA=$(node <<DO:SCRIPTS_PATH>>/update-task-frontmatter.cjs read ".do/projects/<active_project>/phases/$ACTIVE_PHASE/phase.md" active_wave)
