...
<<DO:IF CODEX>>
**Agent authorization:** The caller workflow has authorized spawning <<DO:AGENT_PREFIX>>-council-reviewer
...
<<DO:IF CLAUDE>>
**Orchestrator makes the change directly** using Read/Edit/Write in the main conversation. No `<<DO:AGENT_PREFIX>>-executioner` spawn.
<<DO:ENDIF>>
<<DO:IF CODEX>>
...
Invoke `<<DO:AGENT_PREFIX>>-council-reviewer` with `--type code` pointing at the transient file. Capture the returned verdict and full findings:
...
<<DO:ENDIF>>
<<DO:IF CODEX>>
Spawn the <<DO:AGENT_PREFIX>>-council-reviewer subagent with model `<models.overrides.code_reviewer || models.default>` and the description "Quick-path council review (round 1)". Pass the following prompt:
...
Read the round-1 findings from `quick_council_r1`. Apply the requested changes directly via Read/Edit/Write in the main conversation. Do NOT spawn <<DO:AGENT_PREFIX>>-executioner.
