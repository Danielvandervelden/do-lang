...
If 2 (either method): Display "Update the Approach section in `.do/tasks/<active_task>`, then run /do:continue again." Stop.
  ...
  questions: [
    {
      ...
      options: [
        { label: '/do:task "Fix: <brief description of failure>"' },
        { label: "Handle manually, then run /do:continue" },
      ],
    },
  ],
  multiSelect: false,
...
- If 1 (either method): Display "Run the /do:task command shown above, then return to verify this task."
- If 2 (either method): Display "Fix the issue manually, then run /do:continue to re-run verification."
  ...
  questions: [
    {
      question: "How would you like to proceed?",
      options: [
        { label: '/do:task "Complete: <brief summary>"' },
        { label: "Handle manually, then run /do:continue" },
      ],
    },
  ],
...
Continue to Step V6 with the answer (yes/no).
...
const executionLogEntries = (
  taskMarkdown.match(/^### \d{4}-\d{2}-\d{2}/gm) || []
).length;
