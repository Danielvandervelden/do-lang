...
description: "Wave execution block for /do:project. Spawns <<DO:AGENT_PREFIX>>-executioner against wave.md. Project-pipeline analogue of stage-execute.md — does NOT reference /do:task or active_task."
---

<<DO:IF CODEX>>
**Agent authorization:** The caller workflow has authorized spawning <<DO:AGENT_PREFIX>>-executioner
...
This reference file is loaded by `skills/project.md` `wave next` after `stage-wave-plan-review.md` returns APPROVED. It spawns `<<DO:AGENT_PREFIX>>-executioner` against the wave's `wave.md` file.
...
```bash
node <<DO:SCRIPTS_PATH>>/update-task-frontmatter.cjs check '<wave_path>' stages.execution==complete
```
...
## WE-2: Spawn <<DO:AGENT_PREFIX>>-executioner
...
<<DO:ENDIF>>
<<DO:IF CODEX>>
Spawn the <<DO:AGENT_PREFIX>>-executioner subagent with model `<models.overrides.executioner || models.default>` and the description "Execute wave plan". Pass the following prompt:
...
```bash
node <<DO:SCRIPTS_PATH>>/update-task-frontmatter.cjs read '<wave_path>' stages.execution
```

If `stages.execution` is `complete`, proceed to WE-4. If not, the executioner may have stopped mid-run.

If <<DO:AGENT_PREFIX>>-executioner set `stages.execution: complete`, proceed to WE-4.
