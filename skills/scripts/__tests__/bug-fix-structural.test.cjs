 ...
 *   No file in skills/ should contain "node @scripts/" (shell invocation). Prose
 ...
 *
 * Run: node --test skills/scripts/__tests__/bug-fix-structural.test.cjs
 */
...
describe('Bug 2b fix: no "node @scripts/" shell invocations remain in skills/', () => {
...
describe('Bug 3 fix: config-template.md exists as .md wrapper', () => {
  ...
  before(() => {
    assert.ok(fs.existsSync(mdPath), 'config-template.md must exist in skills/references/');
    content = readExpanded(mdPath);
  });
...
describe('Bug 3 fix: config-template.json has been deleted', () => {
  it('config-template.json must not exist in skills/references/', () => {
