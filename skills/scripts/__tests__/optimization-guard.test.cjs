...
const REFS = path.join(ROOT, 'skills', 'references');
const SCRIPTS = path.join(ROOT, 'skills', 'scripts');
...
// Used for markdown files that contain <<DO:...>> markers — the tests
// check the expanded output (what users actually see after install).
...
describe('optimization-guard: stage-debug.md', () => {
  ...
  const content = readExpanded(path.join(REFS, file));
...
describe('optimization-guard: stage-plan-review.md', () => {
  ...
  it('has PR-4.5 Classify Findings step', () => {
    ...
    const classifyRef = readExpanded(path.join(REFS, 'classify-findings.md'));
    assert.ok(
...
describe('optimization-guard: task.md', () => {
  const file = 'task.md';
  const content = readExpanded(path.join(SKILLS, file));
...
describe('optimization-guard: project.md', () => {
  ...
  it('phase complete preserves planning gate (inline or via stage-phase-transition reference)', () => {
    ...
    const transitionRef = readExpanded(path.join(REFS, 'stage-phase-transition.md'));
    assert.ok(
...
// ─── verifier.md (agent template) ────────────────────────────────────────

describe('optimization-guard: verifier.md', () => {
  const file = 'verifier.md';
  const content = readExpanded(path.join(AGENTS, file));
...
// ─── executioner.md (agent template) ─────────────────────────────────────

describe('optimization-guard: executioner.md', () => {
  const file = 'executioner.md';
  const content = readExpanded(path.join(AGENTS, file));
...
describe('optimization-guard: cross-reference integrity', () => {
  ...
  it('all agent template files exist (bare role names)', () => {
    const requiredAgents = [
      'planner.md', 'plan-reviewer.md', 'code-reviewer.md',
      'council-reviewer.md', 'executioner.md', 'verifier.md',
      'griller.md', 'debugger.md'
    ];
  ...
  it('classify findings only in task pipeline, not wave pipeline', () => {
    const planReview = readExpanded(path.join(REFS, 'stage-plan-review.md'));
    const codeReview = readExpanded(path.join(REFS, 'stage-code-review.md'));
    const wavePlan = readExpanded(path.join(REFS, 'stage-wave-plan-review.md'));
    const waveCode = readExpanded(path.join(REFS, 'stage-wave-code-review.md'));
  ...
  it('classify-findings.md shared reference exists and has required content', () => {
    ...
    const content = readExpanded(classifyFile);
    assertKeywordsPresent(content, [
