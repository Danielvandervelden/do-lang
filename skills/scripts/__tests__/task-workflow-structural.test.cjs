 ...
 *
 * Run: node --test skills/scripts/__tests__/task-workflow-structural.test.cjs
 */
...
const SCRIPT_DIR = path.join(ROOT, 'skills', 'scripts');
...
describe('stage-plan-review.md: classification step', () => {
  ...
  before(() => {
    ...
    content = readExpanded(TASK_STAGE_FILES[0]);
  });
  ...
  it('uses parseCouncilRunnerOutput (not parseFindings) for council output', () => {
    ...
    const combined = fs.existsSync(classifyPath)
      ? content + readExpanded(classifyPath)
      : content;
    assert.ok(
...
describe('stage-code-review.md: classification step', () => {
  ...
  before(() => {
    ...
    content = readExpanded(TASK_STAGE_FILES[1]);
  });
...
describe('do-plan-reviewer.md: severity tags in output contract', () => {
  let content;
  before(() => {
    assert.ok(fs.existsSync(TASK_AGENT_FILES[0]), 'plan-reviewer.md must exist');
    content = readExpanded(TASK_AGENT_FILES[0]);
  });
...
describe('do-code-reviewer.md: severity tags in CHANGES_REQUESTED template', () => {
  let content;
  before(() => {
    assert.ok(fs.existsSync(TASK_AGENT_FILES[1]), 'code-reviewer.md must exist');
    content = readExpanded(TASK_AGENT_FILES[1]);
  });
...
describe('do-council-reviewer.md: bulleted findings format in Step 4', () => {
  let content;
  before(() => {
    assert.ok(fs.existsSync(TASK_AGENT_FILES[2]), 'council-reviewer.md must exist');
    content = readExpanded(TASK_AGENT_FILES[2]);
  });
...
describe('task.md: Step 6 ITERATE bullet permits inline edits by stage reference', () => {
  ...
  before(() => {
    ...
    content = readExpanded(TASK_SKILL_FILES[0]);
  });
