 ...
 *   node optimise-target.cjs skills/scan.md
 ...
 *   skill    — path matches skills/*.md or content has skill frontmatter (name: do:*)
...
function detectTargetType(targetArg, cwd) {
  ...
  // Agent heuristic: agents/*.md (bare role names)
  if (parts[0] === 'agents' && ext === 'md') {
  ...
  // Skill heuristic: skills/*.md or skill frontmatter (name: do:*)
...
function gatherProjectContext(projectPath, effort) {
  if (effort === 'low') {
    ...
    const scriptsDir = path.join(projectPath, 'skills', 'scripts');
    ...
    if (dirExists(scriptsDir)) {
      try {
        scriptFiles.push(...listFilesRecursive(scriptsDir, EXCLUDE_DIRS).map(f => `skills/scripts/${f}`));
      } catch { /* ignore */ }
    }
...
function gatherFileContext(resolvedPath, type, cwd) {
  ...
  if (type === 'skill') {
    ...
    for (const scriptRef of scriptRefs) {
      const scriptPath = path.join(cwd, 'skills', 'scripts', scriptRef);
      if (fileExists(scriptPath)) {
        contextFiles.push(`skills/scripts/${scriptRef}`);
      }
    }
    ...
    for (const refRef of refRefs) {
      const refPath = path.join(cwd, 'skills', 'references', refRef);
      if (fileExists(refPath)) {
        contextFiles.push(`skills/references/${refRef}`);
      }
    }
  }
...
function deriveResearchQueries(type, technologies, effort) {
  ...
  if (effort !== 'low') {
    if (type === 'agent') {
      peer_file_patterns.push('agents/*.md');
    } else if (type === 'skill') {
      peer_file_patterns.push('skills/*.md');
    } else if (type === 'script') {
      peer_file_patterns.push('skills/scripts/*.cjs');
    } else if (type === 'reference') {
      peer_file_patterns.push('skills/references/*.md');
    } else if (type === 'project') {
      peer_file_patterns.push('agents/*.md', 'skills/*.md');
    }
  }
