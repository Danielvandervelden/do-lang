...
<<DO:ENDIF>>
<<DO:IF CODEX>>
Updating via `npm install -g` (not `npm update -g`) is required because `npm update -g` does not trigger the postinstall script. The postinstall script is what copies skill files to `~/.codex/skills/do/`. Skipping postinstall means the running skills stay on the old version even though the package itself was updated.
<<DO:ENDIF>>
...
```
<<DO:IF CLAUDE>>
<<DO:CLAUDE:do commands installed to>><<DO:CODEX:do skills installed to>> /Users/<user>/.claude/commands/do
do agents installed to /Users/<user>/.claude/agents
<<DO:ENDIF>>
<<DO:IF CODEX>>
<<DO:CLAUDE:do commands installed to>><<DO:CODEX:do skills installed to>> /Users/<user>/.codex/skills/do
do agents installed to /Users/<user>/.codex/agents
<<DO:ENDIF>>
```
...
ls ~/.claude/agents/<<DO:AGENT_PREFIX>>-*.md 2>/dev/null | head -1 | grep -q . && echo "agents_ok" || echo "agents_missing"
<<DO:ENDIF>>
<<DO:IF CODEX>>
test -f ~/.codex/skills/do/update.md && echo "codex_ok" || echo "codex_missing"
ls ~/.codex/agents/<<DO:AGENT_PREFIX>>-*.md 2>/dev/null | head -1 | grep -q . && echo "agents_ok" || echo "agents_missing"
<<DO:ENDIF>>
```
...
<<DO:IF CLAUDE>>
  ...
  [ok] ~/.claude/agents/<<DO:AGENT_PREFIX>>-*.md
<<DO:ENDIF>>
<<DO:IF CODEX>>
  [ok] ~/.codex/skills/do/update.md
  [ok] ~/.codex/agents/<<DO:AGENT_PREFIX>>-*.md
<<DO:ENDIF>>
```
