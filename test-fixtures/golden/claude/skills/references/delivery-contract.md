...
The validator script `skills/scripts/validate-delivery-contract.cjs` exports:
...
When `/do:task` is invoked without `--delivery=...` and the project config's `delivery_contract.onboarded` flag is `false`, the onboarding flow triggers. See `skills/references/delivery-onboarding.md` for the three-option flow.
