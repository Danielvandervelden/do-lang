...
The full `/do:task` workflow (5-7 subagent spawns + review rounds) is disproportionate for low-risk mechanical changes. `/do:fast` removes ceremony while keeping context scan, execution, validation, and single code review. Handles 1-6 files for bounded mechanical shared-utility refactors, 1-3 files otherwise.
...
The reference file below handles task-file creation, quick context scan, codex-executioner spawn, stage override, validation, single code review round, completion, and the `/skill-creator` reminder. See `@references/stage-fast-exec.md`.
