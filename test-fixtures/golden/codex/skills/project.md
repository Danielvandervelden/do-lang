---
name: do:project
description: "Multi-phase project orchestration for Codex. Routes to subcommands: new <slug>, phase (new/abandon/complete), wave (new/complete/abandon/next), status, complete, abandon. Use when starting a large greenfield or massive-feature initiative that requires multiple phases and waves of work."
...
```bash
node ~/.codex/skills/do/scripts/read-config.cjs project-config
```
...
2. Call `project-scaffold.cjs project <slug>`:
   ```bash
   node ~/.codex/skills/do/scripts/project-scaffold.cjs project <slug>
   ```
...
1. Call scaffold:
   ```bash
   node ~/.codex/skills/do/scripts/project-scaffold.cjs phase <active_project> <slug>
   ```
...
1. Call:
   ```bash
   node ~/.codex/skills/do/scripts/project-state.cjs abandon phase <slug> --project <active_project>
   ```
...
2. **Precondition check** — every in-scope wave must be completed:
   ```bash
   node ~/.codex/skills/do/scripts/project-state.cjs check waves-complete <active_phase> --project <active_project>
   ```
   If `complete: false`, abort with the `incomplete` list from the JSON output.

3. **State transition:**
   ```bash
   node ~/.codex/skills/do/scripts/project-state.cjs set phase <active_phase> status=completed --project <active_project>
   ```
...
2. Call scaffold:
   ```bash
   node ~/.codex/skills/do/scripts/project-scaffold.cjs wave <active_project> <active_phase> <slug>
   ```
...
1. Call:
   ```bash
   node ~/.codex/skills/do/scripts/project-state.cjs set wave <active_phase>/<slug> status=completed --project <active_project>
   ```
...
1. Call:
   ```bash
   node ~/.codex/skills/do/scripts/project-state.cjs abandon wave <active_phase>/<slug> --project <active_project>
   ```
...
2. Find the next planning wave:
   ```bash
   node ~/.codex/skills/do/scripts/project-state.cjs check next-planning-wave <active_phase> --project <active_project>
   ```
...
4. Set wave status to `in_progress`:
   ```bash
   node ~/.codex/skills/do/scripts/project-state.cjs set wave <active_phase>/<wave_slug> status=in_progress --project <active_project>
   ```
5. Update `phase.md` `active_wave: <wave_slug>` (atomic). Append changelog.
6. **Per-wave confidence rescue:** read `wave.md` confidence score. If below `project_intake_threshold`, spawn codex-griller (pass the project threshold explicitly):

   Spawn the codex-griller subagent with model `<models.overrides.griller || models.default>` and the description "Wave confidence rescue: grill for clarity". Pass the following prompt:
   ...
   **Codex cleanup:** After the codex-griller subagent completes the confidence rescue and its output has been fully consumed, close (dismiss) the codex-griller subagent now to free the thread slot before proceeding to step 7.
...
3. Call α's single-owner abandon operation:
   ```bash
   node ~/.codex/skills/do/scripts/project-state.cjs abandon project <active_project>
...
- **Stage references (called inline via `@references/...`):**
  ...
  - `@references/stage-wave-exec.md` — Spawns `codex-executioner` against `wave.md`
  - `@references/stage-wave-code-review.md` — Spawns `codex-code-reviewer` + council
  - `@references/stage-wave-verify.md` — Spawns `codex-verifier` against `wave.md`
