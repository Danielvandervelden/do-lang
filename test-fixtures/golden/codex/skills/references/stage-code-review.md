...
**Caller contract:** When this stage returns VERIFIED, the task file has been updated with `stage: verification`, `stages.verification: pending`, and `council_review_ran.code: true`. Continue to codex-verifier. If MAX_ITERATIONS, stop and present to the user.
...
**If already ran (exit 1):** Skip this entire stage. Return control to caller (proceed to codex-verifier).
...
```bash
node ~/.codex/skills/do/scripts/council-gate.cjs execution
```
...
Spawn the codex-code-reviewer subagent with model `<models.overrides.code_reviewer || models.default>` and the description "Self-review code". Pass the following prompt:
...
Spawn the codex-council-reviewer subagent with model `<models.overrides.code_reviewer || models.default>` and the description "Council review code". Pass the following prompt:
...
Spawn the codex-code-reviewer subagent with model `<models.overrides.code_reviewer || models.default>` and the description "Self-review code (council disabled)". Pass the following prompt:
...
Collect `self_verdict` (APPROVED/NITPICKS_ONLY/CHANGES_REQUESTED) from codex-code-reviewer and `council_verdict` (APPROVED/NITPICKS_ONLY/CHANGES_REQUESTED) from codex-council-reviewer.
...
When the combined verdict from CR-4 is ITERATE, classify findings before building the brief for codex-executioner.

Load and follow `@references/classify-findings.md`. The self-review output comes from codex-code-reviewer (CHANGES_REQUESTED response).

**No decision matrix for code review.** Code review CR-5 ITERATE always respawns codex-executioner — there is no inline-fix path. Classification is used only to build a prioritized brief: blockers section first ("Must fix"), nitpicks section second ("Should fix"). This helps the executioner prioritize but does not change the branch logic.
...
5. Wait for codex-executioner to complete
