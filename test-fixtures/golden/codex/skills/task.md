---
name: do:task
description: "Start a new piece of work with agent-based workflow. Orchestrates codex-planner, codex-plan-reviewer + codex-council-reviewer (parallel, at plan review), codex-griller (if needed), codex-executioner, codex-code-reviewer + codex-council-reviewer (parallel, at code review), and codex-verifier agents. Creates task file, runs reviews, executes, reviews code, and verifies."
...
```
codex-planner (cyan) → codex-plan-reviewer (green)  ┐ (parallel) → codex-griller (yellow, if needed)
                        codex-council-reviewer (purple)┘                        ↓
                                                              USER APPROVAL
                                                                         ↓
                                                            codex-executioner (red)
                                                                         ↓
                                                 codex-code-reviewer (blue)  ┐ (parallel)
                                                 codex-council-reviewer (purple)┘
                                                                         ↓
                                                            codex-verifier (silver)
```
...
```bash
node ~/.codex/skills/do/scripts/read-config.cjs delivery
```
...
- Abandon it and start new: `node ~/.codex/skills/do/scripts/task-abandon.cjs abandon <filename>`
- Cancel

## Step 3: Read Model Config

```bash
node ~/.codex/skills/do/scripts/read-config.cjs models
```

Store result for subagent spawning. Default to sonnet if not configured.
...
- Leave all other `{{PLACEHOLDER}}` fields as-is — codex-planner fills them in Step 5.
...
## Step 5: Spawn codex-planner

Spawn planner to fill in the task file:

Spawn the codex-planner subagent with model `<models.overrides.planner || models.default>` and the description "Plan task: <description>". Pass the following prompt:
...
**Codex cleanup:** The codex-planner subagent has completed and its output has been fully consumed. Close (dismiss) the codex-planner subagent now to free the thread slot before proceeding to plan review.
...
```bash
node ~/.codex/skills/do/scripts/update-task-frontmatter.cjs read '.do/tasks/<active_task>' confidence
...
Spawn the codex-griller subagent with model `<models.overrides.griller || models.default>` and the description "Grill user for clarity". Pass the following prompt:
...
## Step 9: Spawn codex-executioner
...
- **PASS**: Task marked complete by codex-verifier; continue to Step 12
...
**Codex cleanup:** The codex-verifier subagent has completed and its output has been fully consumed. Close (dismiss) the codex-verifier subagent now to free the thread slot before proceeding to completion.
...
```bash
node ~/.codex/skills/do/scripts/update-task-frontmatter.cjs read '.do/tasks/<active_task>' stage
```
...
- **If stage is `complete`**: Display brief confirmation:
  ...
  ```
- **If stage is not `complete`** (UAT failed, verification failed, user chose to loop back): Display codex-verifier's output as-is — it already contains the user's next steps.

---

## Failure Handling

Any subagent failure returns immediately to user with:
